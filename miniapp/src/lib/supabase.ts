...
export async function getApprovedProfilesWithPhotos(
  ...
  const supabase = getSupabase()
  console.log('[getSwipeProfiles] Starting for user:', excludeUserId, 'city filter:', city || 'none')
  ...
  console.log('[getSwipeProfiles] Total visible profiles:', data?.length || 0, 'Already swiped:', swipedIds.length, 'Matches:', matchedIds.size)
