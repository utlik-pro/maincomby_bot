...
**Текущий хостинг:** [vdska.ru](https://vdska.ru) (VDS/VPS)

### 🚀 Автоматический деплой БЕЗ SSH

После начальной настройки обновляйте бота просто через `git push`:
...
**Документация:**
- **[QUICK_DEPLOY_GUIDE.md](./QUICK_DEPLOY_GUIDE.md)** - Самый быстрый способ (5 минут настройки)
- **[AUTO_DEPLOY_README.md](./AUTO_DEPLOY_README.md)** - Обзор всех способов
- **[HOSTING_INFO.md](./HOSTING_INFO.md)** - Информация о сервере и управлении

**Способы автодеплоя:**
1. **Git Cron** (рекомендуется) - обновление каждые 5 минут
2. **Webhook** - обновление через ~30 секунд
3. **Watchtower** - Docker auto-update для CI/CD

### 📊 Текущий статус

- ✅ **306 пользователей**
- ✅ **247 регистраций** на мероприятия
- ✅ Активно используется сообществом
- 🚀 Автодеплой настроен

**Логи и мониторинг:**
```bash
# Просмотр логов
ssh root@104.253.1.54 "docker logs maincomby_bot -f"

# Статус бота
ssh root@104.253.1.54 "docker ps | grep maincomby_bot"
```
...
- [SIMPLE_DEPLOY.md](./SIMPLE_DEPLOY.md) - Альтернативные способы деплоя
